...
public class CookieAdd extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1. Cookie 객체를 생성한다. ==> '쿠키 key값' 와 '쿠키의 value값'을 문자열로 지정해서 생성한다.
		// 형식) Cookie cookie변수 = new Cookie("쿠키의 key","쿠키의 value");
		//       ==>> 쿠키 value 값이 한글일 경우에는 URLEncoder.encode()메서드로 인코딩후 저장 
		...
		// 쿠키변수.setMaxAge(유지시간); == > 단위(초)   (-1: 브라우저가 종료될때 까지 유지(기본값)), 0: 즉시 삭제된다.)
		// 쿠키변수.setSecure(보안여부); ==> true : 적용, false : 미적용(기본값)
		
		
		// 3. Response 객체를 이용해서 쿠키를 웹브라우저로 보내면 웹브라우저가 쿠키를 받아서 저장한다.
		...
		out.println("<html><head><meta charset='utf-8'><title>Cookie데이터 생성</title></head>");
		out.println("<body>");
		out.println("<h3>Cookie 데이터가 저장되었습니다</h3>");
		
		out.println("<a href='" + request.getContextPath() +"/basic/cookie/cookieTest.jsp'> 시작문서로가기</a>");
		out.println("</body>");
		out.println("</html>");
